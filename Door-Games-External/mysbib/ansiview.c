...
*
****************************************************************************/
// Changes by LaSys MMXVIII
//
// v 0.1
//
// 20180224 - Changed constant at pageup() to 1919 - leaded to core dump
// 20180224 - Fixed the gotobookmark to test for file existance
// 20180224 - added 'e' for goto end
// 20180224 - added searchforterm to look for word LORD
// 20180224 - added soubroutine render from position
// 20180224 - improved stability of home, end etc.
//
#include <stdio.h>
#include <string.h>
#include <curses.h>
...
int splos = 0;
char username[64]; 
char filename[32] = "_bible.txt";
char bookfile[128];

unsigned int editbuffer[5000000];	/* Max 400000 lines = 5000000 */
                                  /* yes we can do 400000 line ansi's :) */
char searchtext[5000001];
char revsearchtext[5000001];
const char *loc = searchtext;
const char *ploc = revsearchtext;
const char *p = &loc[5000001];
const char *pp = &ploc[5000001];
int wordOccurence = 0;
...
void ansiscrolldown () {
  if (pos>(ROWS*COLS)-1 && pos<maxread) {
    pos = pos - (ROWS*COLS) + (COLS);
    renderFromPos();
    ishome = 0;
  }
...
int calcline(int charpos) {
  int charline = 0;
  charline = charpos/80;
  return (charline);
}

void initsearch() { // lasys 20180224
  for (int i=0;i<maxread;i++) {
    searchtext[i]=editbuffer[i]; 
  }  
}

void searchforterm() { // lasys 20180224
  //char *loc;
  int charpos = 0;
  if (strstr(loc,"Chapter")) {
    loc=strstr(loc,"Chapter");
    loc++;
    wordOccurence++;
    charpos=loc-searchtext;
    int line=calcline(charpos);
    pos=(line-2)*COLS;
    renderFromPos();
    printf("Line: %d Chapter: %d",pos/80,wordOccurence);
  }
}

void searchforday() { // lasys 20180224
  int charpos = 0;
  if(strstr(loc,"*******")) { 
    loc=strstr(loc,"*******");
    loc=loc+24;
    ...
    pos=(line-2)*COLS;
    renderFromPos();
    printf("Line: %d Day: %d",pos/80,wordOccurence);
  }
}
...
//    ("\e[0;1;32;40m Linux Ansi Viewer by \e[0;1;37;40mBen Fowler\e[0m\n\n\r");
  ...
  int esc=27;
  printf ("%c[[20h",esc);		/* turn cursor on */
  printf ("%c[201",esc);		/* turn cursor on */
  printf ("%cc",esc);		/* reset terminal */
  printf ("%c[0m",esc);		/* turn cursor on */
  printf ("%c[<",esc);		/* turn cursor on */
  printf ("%c[20h",esc);		/* turn cursor on */
  printf ("%c[1;25r",esc);		/* reset and refresh the screen */
  endwin();
  printf("Returning to BBS...\n");
}				/* end scrollit() */


void cmdrun() {
   printf("Takes exactly two arguments: 1) file and 2) username\n");
}
...
main (int argc, char *argv[])
{
  ...
  if (argc>0) {
    sprintf(username,"%s_%s",argv[2],argv[1]);
  }
  ...
  if ((fp = fopen (argv[1], "r")) != NULL)
    {
      ...
      while (((c = getc (fp)) != -1) && (c != 26) && (pos < 5000000))
	{
	  /* 26 = dos eof */
